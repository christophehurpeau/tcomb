      ...
      types.forEach(function (type) {
        if (isType(type)) {
          try {
            ...
            assert.fail('Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/')
                + ': ' + err.message.substr('[tcomb] '.length));
          }
        } else {
          assert(
            value instanceof type,
            function () { return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/'); }
          );
        }
      });
    }
